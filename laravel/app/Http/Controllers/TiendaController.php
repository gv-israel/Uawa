...
class TiendaController extends Controller
{
    ...
    public function index(Request $request)
    {
    	if($request)
    	{
    		...
    		return view('tienda', ["articulos"=>$articulos,"textoBusqueda"=>$consulta]);
    	}
    ...
    public function store(ArticuloFormRequest $request)
    {
        ...
        $i = 0;
        //RECORRE NUMERO DE VARIACIONES
        $cantidad = Input::get('nombreVariacion');
        foreach($cantidad as $can) {


        if($can!=NULL)
        {
            //CREA NUEVA VARIACION DEL ARTICULO
            $variacion = new ArticuloVariacion;
            $variacion->deArticulo=$articulo->id;
            $variacion->nombre=$can;
            $variacion->stock=Input::get('stockVariacion.'.$i);
            $variacion->save();

            //CREA REGISTROS DE IMAGENES
            $imagenes = $request->file('imagenesVariacion.'.$i);
            foreach($imagenes as $img)
            {
                if($img)
                {
                    ...
                    //CREA REGISTRO
                    $imag = new ArticuloImagen;
                    $imag->deVariacion=$variacion->id;
                    $imag->imagen=$nombre;

                        //BUSCA SI EXISTE ALGUN DESTACADO DEL ARTICULO
                        $destacadoExiste=DB::table('articulos as a')
                        ->join('categorias as c', 'c.id', '=', 'a.deCategoria')
                        ->join('articulos_variaciones as av', 'av.deArticulo', '=', 'a.id')
                        ->join('articulos_imagenes as ai', 'ai.deVariacion', '=', 'av.id')
                        ->select('ai.destacado')
                        ->where('ai.destacado','=','1')
                        ->where('a.id','=',$articulo->id)
                        ->count();
    ...
    public function show($slug)
    {
        ...
        $imagenes=DB::table('articulos as a')
        ->join('articulos_imagenes as ai', 'ai.deArticulo', '=', 'a.id')
        ->where('a.slug','=',$slug)
        ->get();

        return view('articulo', ["articulo"=>$articulo,"imagenes"=>$imagenes]);
    }
    ...
    public function update(CategoriaFormRequest $request, $id)
    {
    	...
    	$articulo->deCategoria=$request->get('categoria');
        $articulo->codigo=rand(15);
        $articulo->nombre=$request->get('nombre');
        $articulo->descripcion=$request->get('descripcion');
